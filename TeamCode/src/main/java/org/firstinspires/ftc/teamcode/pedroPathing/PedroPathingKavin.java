...
@Autonomous (name = "PedroPathingKavinComplete", group = "Autonomous", preselectTeleOp = "TELEOP_JAVA")
public class PedroPathingKavin extends OpMode {
    ...
    private void statePathUpdate(){
        switch (pathState){
            ...
            case DriveTo_1:
                ...
                if (pathTimer.getElapsedTimeSeconds() > 0.4 && !flywheelStarted) {
                ...
                if (!follower.isBusy() && pathStarted && pathTimer.getElapsedTimeSeconds() > 0.2) {
                    setPathState(PathState.Launch_Element_1);
                }
                break;

            case Launch_Element_1:
                launcher.update();
                if (!flywheelStarted) {
                ...
                // Checks internal busy-flag AND waits until 2 seconds are complete
            ...
            case DriveTo_2Pickup:
                ...
                if (pathTimer.getElapsedTimeSeconds() >= 0 && !intakeStarted) {
            ...
            case DriveTo_3:
                ...
                if (!follower.isBusy() && pathStarted && pathTimer.getElapsedTimeSeconds() > 0.3) {
                    setPathState(PathState.Launch_Element_2);
                }
                break;


            case Launch_Element_2:
                launcher.update();
                if (!flywheelStarted) {
                    launcher.setTargetRpm(2200);
                    launcher.setAnglePosition(0.14);
            ...
            case DriveTo_5:
                ...
                if (!follower.isBusy() && pathStarted && pathTimer.getElapsedTimeSeconds() > 0.3) {
                    setPathState(PathState.Launch_Element_3);
                }
                break;

            case Launch_Element_3:
                launcher.update();
                if (!flywheelStarted) {
                    launcher.setTargetRpm(2200);
                    launcher.setAnglePosition(0.14);
